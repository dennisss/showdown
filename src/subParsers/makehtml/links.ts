import { _hashHTMLSpan, encodeEmailAddress, escapeCharactersCallback, Event, isString, isUndefined, regexes } from '../../helpers';
 ...
 *
 * @param {RegExp} rgx
 * @param {string} evtBaseName
 * @param {{}} options
 * @param {{}} globals
 * @returns {Function}
 */
function replaceAnchorTag (rgx: RegExp, evtBaseName: string, options: ConverterOptions, globals: ConverterGlobals, emptyCase: boolean = false) {
  ...
  return function (wholeMatch: string, text: string, id: string, url: string, m5: string, m6: string, title: string|null) {
    ...
    var evt = createEvent(rgx, evtBaseName + '.captureStart', wholeMatch, text, id, url, title, options, globals);
...
export function makehtml_links_angleBrackets (text: string, options: ConverterOptions, globals: ConverterGlobals) {
  ...
  text = text.replace(urlRgx, function (wholeMatch: string, url: string, urlStart: string) {
    ...
    var evt = createEvent(urlRgx, evtBaseName + '.captureStart', wholeMatch, originalUrl, null, url, null, options, globals);
...
export function makehtml_links_naked (text: string, options: ConverterOptions, globals: ConverterGlobals) {
  ...
  text = text.replace(urlRgx, function (wholeMatch, leadingMDChars, url, urlPrefix) {
    ...
    originalUrl = originalUrl.replace(regexes.asteriskDashTildeAndColon, escapeCharactersCallback);
